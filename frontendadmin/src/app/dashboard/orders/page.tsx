            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4 transition-transform hover:scale-[1.01]">
                    ...
                    <div className="min-w-0 flex-1">
                        <p className="text-[10px] sm:text-xs md:text-sm font-bold text-gray-500 uppercase truncate" title="Total Orders">Total Orders</p>
                        <h3 className="text-xl sm:text-2xl font-black text-gray-900 truncate">{filteredOrders.length}</h3>
                    </div>
                </div>
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4 transition-transform hover:scale-[1.01]">
                    ...
                    <div className="min-w-0 flex-1">
                        <p className="text-[10px] sm:text-xs md:text-sm font-bold text-gray-500 uppercase truncate" title="Pipeline Value">Pipeline Value</p>
                        <h3 className="text-xl sm:text-2xl font-black text-gray-900 truncate">₹ {filteredOrders.reduce((sum, order) => order.status !== 'cancelled' ? sum + Number(order.total_amount || 0) : sum, 0).toLocaleString()}</h3>
                    </div>
                </div>
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4 transition-transform hover:scale-[1.01]">
                    ...
                    <div className="min-w-0 flex-1">
                        <p className="text-[10px] sm:text-xs md:text-sm font-bold text-gray-500 uppercase truncate" title="Advance Collected">Advance Collected</p>
                        <h3 className="text-xl sm:text-2xl font-black text-gray-900 truncate">₹ {filteredOrders.reduce((sum, order) => order.status !== 'cancelled' ? sum + Number(order.advance_paid || 0) : sum, 0).toLocaleString()}</h3>
                    </div>
                </div>
            </div>
